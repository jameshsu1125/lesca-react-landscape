...
const Landscape = (props: LandscapeProps) => {
  ...
  useEffect(() => {
    OrientationChange.addEventListener((e) => {
      if (deviceWidth !== false) {
        const dw = deviceWidth || 750;
        if (typeof dw === 'number') {
          const s = window.screen.width / dw;
          const p: Elements | null = document.querySelector('meta[name="viewport"]');
          if (p) {
  ...
  return <>{state && <div style={{ ...defaultStyle, ...props.style }} />}</>;
};
